...
*make_basic_test_top_array_string_container (yajl_val tree, const struct parser_context *ctx, parser_error *err)
{
    ...
    *err = NULL;
    ...
    if (alen == 0)
      return NULL;
    ptr = calloc (1, sizeof (basic_test_top_array_string_container));
    if (ptr == NULL)
      return NULL;
                       ...
                       parser_error *err)
{
    ...
    *err = NULL;
    ...
    for (i = 0; i < ptr->len; i++)
      {
        {
            stat = yajl_gen_string ((yajl_gen)g, (const unsigned char *)(ptr->items[i]), strlen (ptr->items[i]));
            if (stat != yajl_gen_status_ok)
                GEN_SET_ERROR_AND_RETURN (stat, err);
